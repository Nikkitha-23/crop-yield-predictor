...
def predict_fertilizer(soil_type, soil_ph, soil_moisture,
    ...
    try:
        ...
        # Support multiple possible target encoder key names
        target_le = (
            encoders.get('__target__') or
            encoders.get('target') or
            encoders.get('Recommended_Fertilizer')
        )

        if target_le is None:
            return {'error': 'Target encoder not found in encoders file'}
    ...
    input_data = {
        ...
        'Fertilizer_Used_Last_Season': safe_encode(fert_last_season, 'Fertilizer_Used_Last_Season'),
    ...
    try:
        ...
        top3 = [
            {
                'fertilizer'  : target_le.inverse_transform([i])[0],
                'probability' : round(float(pred_prob[i]) * 100, 1)
            }
            for i in top3_idx
        ]
